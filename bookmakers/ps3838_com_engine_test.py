...
MAIN_LOOP = 1
limit_time = 24  # limit time in hours
mk = 0  # Today
...
class Ps3838Com:
    ...
    async def lets_login(self):
        ...
        payload = {"email": "", "password": "", "loginType": 1, "remember": False}
    ...
    async def bound_live(self, sem, session, sport_id, mk):
        prematch = []
        try:
            prematch = await self.get_live_(sem, session, sport_id, mk)
        except OSError as e:
            logging.info(f'{e}')
        try:
            if len(prematch) == 0:
    ...
    async def get_live_(self, sem, session, sport_id, mk):
        # for mk in [1, 0]:
            if mk == 1:
                date_ = datetime.today().strftime('%Y-%m-%d')
                ...
                date_ = ''
                url = f"https://761gsge.tender88.com/sports-service/sv/compact/" \
                      f"events?_g=1&btg=1&c=&cl=100&d=&ev=&g=&hle=true" \
                      f"&l=3&lg=&lv=&me=0&mk={mk}&more=false&o=1&ot=1&pa=0&pn=-1" \
                      f"&sp={sport_id}&tm=0&v=0&wm=&locale=en_US&_={int(time.time() * 1000)}" \
                      f"&withCredentials=true"
    ...
    async def bet_attribute_none(self, match):
        betAttribute = {
            'info': {
                'bookmaker': self.BOOKIE,
                'match_id': match[0],
    ...
    async def loop_markets(self, markets_, match):
        ...
        try:
            ...
            # raise
            logging.debug(f"no market yet")
            return
        # print(markett[8]['1'])
        # print(len(markett[8]['1']))
        if markett[8]['1'][2] is not None:
            for ii, price in enumerate(markett[8]['1'][2]):
                # print(price)
                type_name = 'First Half 1X2'
                if markett[8]['1'][2][2] is None and sport_id != 33:
                    type_name = 'First Half 12'
                elif markett[8]['1'][2][2] is None and sport_id == 33:
                    type_name = '1st Set'
                if ii >= 3:
                    break
                if price is None:
    ...
    async def get_overtime(self, markett):
        # print(markett)
        try:
            for totals in markett[8]['6'][1]:
                for ii, price in enumerate(totals):
                    if ii >= 2:
                        break
                    scanner_format_bet = await self.get_totals(ii, totals)
                    # converted_m_list.append(scanner_format_bet)
        except (KeyError, ValueError):
            logging.info(f"no market yet")
            return
    ...
    async def convert_odds_from_us_eu_format(self, us_odds):
        eu_odds = 0
        if us_odds < 0:
            eu_odds = 100/ -us_odds + 1
        elif us_odds > 0:
            eu_odds = us_odds / 100 + 1
        return round(eu_odds, 3)

    async def check_type(self, type_: str):
        if type_ == 'moneyline':
            return True
        if type_ == 'total':
            return True
            ...
            for mk in [1, 0]:
                try:
                    task = asyncio.ensure_future(
                        self.bound_live(sem, self.session, sport_id, mk))
                    tasks.append(task)
                except Exception as e:
                    print(e)
        responses = await asyncio.gather(*tasks)
    ...
    def loop2(self, matches, index_coins):
        ...
        for all in all_sport_markets:
            dicts[list(all)[0]] = all[list(all)[0]]

        loop = asyncio.get_event_loop()
        future = asyncio.ensure_future(
            self.run_(matches, index_coins, dicts))
...
if __name__ == '__main__':
    ...
    while True:
        try:
            ...
            future = asyncio.ensure_future(ps.get_live())
